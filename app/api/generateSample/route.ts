...
export async function POST(request: Request) {
  try {
    ...
    if ( !name || !fontSize || !color || textY === undefined) {
      return NextResponse.json({ message: 'Invalid dimensions, missing name, or missing text properties' }, { status: 400 });
    }
    ...
    // Download and register a Google font (Roboto as an example)
